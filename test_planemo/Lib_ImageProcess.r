...
define_output_directory <- function(Output_Dir, ImPath, TypePCA) {
  Image_Name <- tools::file_path_sans_ext(basename(ImPath))
...
extract.big_raster <- function(ImPath, rowcol, MaxRAM=.50){
  ...
  sampleList = lapply(unique(rowcol$block), function(iblock){
    ...
    values <- stars::read_stars(ImPath, RasterIO =list(nXSize=nXSize, nYOff=rr[1], nYSize=nYSize),proxy = FALSE, driver=driver)[[1]]
...
get_BB_from_Vector <- function(path_raster,path_vector,Buffer = 0){
  ...
  BB <- rgeos::gBuffer(spgeom = as(sf::st_read(dsn = path_vector,quiet = T), "Spatial"),width=Buffer,byid = TRUE)
...
get_random_subset_from_image <- function(ImPath, MaskPath, nb_partitions, Pix_Per_Partition, kernel=NULL,MaxRAM = 0.5) {
  ...
  if ((!MaskPath == "") & (!MaskPath == FALSE)) {
    mask <- t(matrix(raster::values(raster::raster(MaskPath)), ncol= nlines, nrow = nsamples))
  } else {
...
get_polygonCoord_from_Shp <- function(path_SHP,path_Raster,IDshp=NULL){
  ...
  if (file.exists(paste(tools::file_path_sans_ext(path_SHP),'.shp',sep=''))){
    ...
    if (!raster::compareCRS(raster::raster(path_Raster), Plot)){
...
#' @importFrom tools file_ext tools::file_path_sans_ext
...
VectorInRasterFootprint <- function(PathVect, PathRaster, buffer=NULL, PathVectOut=NULL, WriteShp = TRUE){
  ...
  if (fileext=='shp'){
    NameVect <- tools::file_path_sans_ext(basename(PathVect))
  ...
  RastOGR_proj <- raster::projection(raster::raster(PathRaster))
  ...
  vectSF <- sf::st_read(PathVect)[plotsinraster,]
  ...
  if (WriteShp==TRUE){
    if (is.null(PathVectOut)){
      PathVectOut <- paste(tools::file_path_sans_ext(PathVect),'_buffer',sep = '')
    }
...
write_StarsStack <- function(StarsObj, dsn, BandNames=NULL, datatype='Float32'){
  ...
  raster::hdr(raster::raster(dsn), format = "ENVI")
